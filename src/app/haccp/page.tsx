...
export default function HaccpPage() {
  ...
  const cols: string[][] = board === 'capa'
    ? [['capa_number', 'CAPA #'], ['source', 'Source'], ['description', 'Problem'], ['corrective_action', 'Corrective Action'], ['status', 'Status'], ['capa_date', 'Date']]
              ...
              : rows.length === 0 ? <tr><td colSpan={cols.length + 1} className="px-4 py-8 text-center text-gray-400 text-sm">No entries yet.</td></tr>
                : rows.map((row) => (
                  <tr key={row.id} className="border-b border-[#F1F2F6] hover:bg-[#FAFBFF] align-top">
                    {cols.map((c) => <td key={c[0]} className="px-4 py-3 text-sm text-gray-700"><div className="line-clamp-3 max-w-xs">{fmt(row[c[0]])}</div></td>)}
