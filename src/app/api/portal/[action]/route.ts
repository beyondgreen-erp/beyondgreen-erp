...
async function sendClientEmail(to: string, subject: string, inner: string): Promise<boolean> {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: `beyondGREEN <${FROM_EMAIL}>`, to: [to], cc: [NOTIFY], reply_to: [NOTIFY], bcc: ['Rudy@beyondgreenbiotech.com'], subject, html: shell(inner) }),
    })
    return res.ok
  } catch { return false }
}
...
export async function POST(req: NextRequest, { params }: { params: { action: string } }) {
  ...
  if (action === 'message') {
    ...
    if (RESEND_API_KEY) {
      ...
      await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: `beyondGREEN <${FROM_EMAIL}>`, to: [NOTIFY], reply_to: client.email || undefined, bcc: ['Rudy@beyondgreenbiotech.com'], subject: `Client message — ${company || client.name || client.email}`, html }) }).catch(() => {})
    }
    return NextResponse.json({ ok: true })
  }
  ...
  if (action === 'comment-add') {
    ...
    if (RESEND_API_KEY) {
      ...
      await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: `beyondGREEN <${FROM_EMAIL}>`, to: [NOTIFY], reply_to: client.email || undefined, bcc: ['Rudy@beyondgreenbiotech.com'], subject: `Client comment — ${own.label || company || client.email}`, html }) }).catch(() => {})
    }
  ...
  if (action === 'rfq-update') {
    ...
    if (RESEND_API_KEY) {
      ...
      await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: `beyondGREEN <${FROM_EMAIL}>`, to: [NOTIFY], reply_to: client.email || undefined, bcc: ['Rudy@beyondgreenbiotech.com'], subject: `RFQ edited by ${company || client.name || 'client'}`, html }) }).catch(() => {})
    }
    return NextResponse.json({ ok: true })
  }
  ...
  if (action === 'message-user') {
    ...
    if (RESEND_API_KEY) {
      ...
      await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: `beyondGREEN <${FROM_EMAIL}>`, to: targets, cc: [NOTIFY], reply_to: client.email || undefined, bcc: ['Rudy@beyondgreenbiotech.com'], subject: isGroup ? `New team message from ${company || client.name || 'client'}` : `New message from ${company || client.name || 'client'}`, html }) }).catch(() => {})
    }
    return NextResponse.json({ ok: true })
  }
