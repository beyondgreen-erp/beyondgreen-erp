...
async function buildPlaybook(): Promise<string> {
  const { data } = await supabase.from('customer_outreach').select('subject, opened_at, response_received, sent_at, parent_outreach_id').not('sent_at', 'is', null).is('parent_outreach_id', null).limit(500);
  const rows = (data || []) as any[];
  if (rows.length < 4) return '';
  const winners = rows.filter((r: any) => r.response_received || r.opened_at).map((r: any) => r.subject).filter(Boolean).slice(0, 8);
  const losers = rows.filter((r: any) => !r.opened_at && !r.response_received).map((r: any) => r.subject).filter(Boolean).slice(0, 8);
  if (!winners.length && !losers.length) return '';
  let s = 'Use what has actually worked in our own past campaigns. ';
  if (winners.length) s += 'Subjects that earned opens or replies (emulate their style, length, tone, and specificity): ' + winners.map((x: string) => '"' + x + '"').join('; ') + '. ';
  if (losers.length) s += 'Subjects that were ignored (avoid this style): ' + losers.map((x: string) => '"' + x + '"').join('; ') + '. ';
  s += 'Write the subject line in the proven winning style.';
  return s;
}
...
Return ONLY a JSON object: {"subject": string, "body": string}.${playbook ? '\n\nPERFORMANCE PLAYBOOK (learn from our own past results): ' + playbook : ''}`;
