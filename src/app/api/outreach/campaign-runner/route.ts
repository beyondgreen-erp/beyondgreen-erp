...
export async function GET(req: NextRequest) {
  ...
  for (const row of (queued || []) as any[]) {
    try {
      ...
      await sendViaGraph(token, { to: row.to_email, subject: draft.subject, html: buildHtml(draft.body) });
  ...
  for (const row of (dueRows || []) as any[]) {
    try {
      ...
      // Log this follow-up as its own history record.
