  ...
  return (
    <div className="p-4 md:p-8 min-h-screen">
      ...
      <div className="rounded-xl border border-[#E4E6EE] bg-white overflow-x-auto">
        ...
        :<table className="w-full min-w-[600px] text-sm"><thead><tr className="border-b border-[#E4E6EE]">{['Title','Category','Version','Effective Date','Review Date','Status','Owner','Customer','Vendor'].map(h=><th key={h} className="text-left text-xs font-semibold text-gray-500 px-5 py-3">{h}</th>)}</tr></thead>
        <tbody>{filtered.map((r,i)=>{
          const reviewDue=isDueForReview(r.review_date,r.status)
          return <tr key={r.id} onClick={()=>openEdit(r)} className={`border-b border-[#E4E6EE]/60 last:border-0 cursor-pointer transition-colors ${reviewDue?'bg-amber-500/5 hover:bg-amber-500/10':i%2===0?'hover:bg-[#F9FAFB]':'bg-[#F5F6FA]/10 hover:bg-[#F9FAFB]'}`}>
            <td className="px-5 py-3.5 text-[#1A1D2E] font-medium">{r.title}</td>
