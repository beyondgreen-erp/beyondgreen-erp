...
function buildRecord(d: any) {
  const NL = String.fromCharCode(10)
  const fmt = (r: any) => 'PO ' + (r.po_number || '-') + ': ' + [(r.short_qty || 0) > 0 ? ('SHORT ' + r.short_qty) : '', (r.over_qty || 0) > 0 ? ('OVER ' + r.over_qty) : '', (r.damaged_qty || 0) > 0 ? ('DAMAGED ' + r.damaged_qty) : ''].filter(Boolean).join(', ')
  return [
    'EXCEPTION REPORT - Delivery ' + (d.delivery_no || '-'),
    'Date: ' + (d.report_date || '-') + '  |  CenterPoint: ' + (d.centerpoint || '-') + '  |  Carrier: ' + (d.carrier || '-') + '  |  Trailer: ' + (d.trailer || '-'),
    'Uploaded by ' + (d.uploadedBy || 'unknown') + (d.uploadedAt ? (' on ' + String(d.uploadedAt).slice(0, 10)) : ''),
    'Total PO lines: ' + d.count + '  |  Lines with exceptions: ' + d.exCount,
    '',
    'EXCEPTIONS:',
    d.ex.length ? d.ex.map(fmt).join(NL) : '(none)',
  ].join(NL)
}
...
export default function BusinessReportsPage() {
  ...
  const deliveries = useMemo(() => {
    ...
    for (const r of rows as any[]) {
      const key = r.delivery_no || '(no delivery #)'
      if (!map.has(key)) map.set(key, [])
      map.get(key)!.push(r)
    }
    const arr = Array.from(map.entries()).map(([delivery_no, list]) => {
      const ex = list.filter((r: any) => (r.short_qty || 0) > 0 || (r.over_qty || 0) > 0 || (r.damaged_qty || 0) > 0)
      const first: any = list[0] || {}
      const uploadedAt = list.map((r: any) => r.created_at || '').sort().reverse()[0] || ''
      return { delivery_no, ex, count: list.length, exCount: ex.length, report_date: first.report_date || '', centerpoint: first.centerpoint || '', carrier: first.carrier_name || '', trailer: first.trailer_number || '', uploadedBy: first.created_by || '', uploadedAt, file: (list.find((r: any) => r.exception_report_file) || {}).exception_report_file || null }
    })
    arr.sort((a, b) => (b.uploadedAt || b.report_date || '').localeCompare(a.uploadedAt || a.report_date || ''))
    return arr
  }, [rows])
  ...
  async function handleUpload(file: File) {
    setUploading(true); setErr(''); setMsg(''); setUploadRecord('')
    try {
      ...
      <div className="bg-white rounded-2xl border border-[#E4E6EE] overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-6 py-4 border-b border-[#E4E6EE]">
          ...
          <div className="flex items-center gap-2">
            ...
            <input ref={fileRef} type="file" accept=".pdf,.xlsx,.xls,.csv,image/*" onChange={onFile} className="hidden" />
        ...
        {uploadRecord && (
          <div className="mx-6 mt-3 border border-indigo-200 bg-indigo-50/40 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-indigo-100">
              <span className="text-sm font-semibold text-[#1A1D2E]">Upload record — copy &amp; share with the team</span>
              <div className="flex items-center gap-2">
                <button onClick={() => { navigator.clipboard.writeText(uploadRecord); setMsg('Upload record copied to clipboard.') }} className="inline-flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors"><i className="ti ti-copy" />Copy</button>
                <button onClick={() => setUploadRecord('')} className="text-xs text-gray-500 hover:text-gray-700 px-2 py-1.5">Dismiss</button>
              </div>
            </div>
            <pre className="px-4 py-3 text-xs text-[#1A1D2E] whitespace-pre-wrap font-mono max-h-72 overflow-y-auto">{uploadRecord}</pre>
          </div>
        )}

        {deliveries.length > 0 && (
          <div className="mx-6 mt-3 border border-[#E4E6EE] rounded-xl overflow-hidden">
            <button onClick={() => setShowHistory(v => !v)} className="w-full flex items-center justify-between px-4 py-2.5 bg-[#FAFBFF] hover:bg-[#F1F2F6] transition-colors">
              <span className="text-sm font-semibold text-[#1A1D2E]"><i className="ti ti-history mr-1.5" />Upload history — {deliveries.length} deliver{deliveries.length === 1 ? 'y' : 'ies'}</span>
              <i className={"ti text-gray-400 " + (showHistory ? "ti-chevron-up" : "ti-chevron-down")} />
            </button>
            {showHistory && (
              <div className="divide-y divide-[#F1F2F6] max-h-96 overflow-y-auto">
                {deliveries.map(d => (
                  <div key={d.delivery_no} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-[#1A1D2E] truncate">Delivery {d.delivery_no} <span className="text-gray-400 font-normal">· {d.report_date || '—'}</span></p>
                      <p className="text-xs text-gray-500 truncate">{d.count} lines · {d.exCount} with exceptions{d.uploadedBy ? ' · by ' + d.uploadedBy : ''}</p>
                    </div>
                    <button onClick={() => { navigator.clipboard.writeText(buildRecord(d)); setMsg('Upload record for delivery ' + d.delivery_no + ' copied.') }} className="inline-flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold px-3 py-1.5 rounded-lg transition-colors whitespace-nowrap"><i className="ti ti-copy" />Copy record</button>
                    {d.file && <button onClick={() => viewFile(d.file)} className="inline-flex items-center gap-1.5 border border-[#E4E6EE] text-[#1A1D2E] text-xs font-semibold px-3 py-1.5 rounded-lg hover:bg-[#F5F6FA] transition-colors whitespace-nowrap"><i className="ti ti-file-text" />View file</button>}
                  </div>
                ))}
              </div>
            )}
