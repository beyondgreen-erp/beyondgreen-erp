...
interface Quotation { id: string; quote_number: string; customer_id: string | null; quote_date: string | null; expiry_date: string | null; status: string; total: number; notes: string | null; is_active: boolean }
...
const empty = { quote_number:'', customer_id:'', quote_date:'', expiry_date:'', status:'Draft', total:'', notes:'' }
type F = typeof empty
...
export default function QuotationsPage() {
  ...
  function openEdit(r:Quotation){setEditing(r);setForm({quote_number:r.quote_number,customer_id:r.customer_id??'',quote_date:r.quote_date??'',expiry_date:r.expiry_date??'',status:r.status,total:r.total?String(r.total):'',notes:r.notes??''});setErr('');setOpen(true)}
  ...
  async function save(){
    ...
    const p={quote_number:form.quote_number.trim(),customer_id:form.customer_id||null,quote_date:form.quote_date||null,expiry_date:form.expiry_date||null,status:form.status,total:parseFloat(form.total)||0,notes:form.notes.trim()||null}
    const{error}=editing?await sb.from('quotations').update({...p,updated_at:new Date().toISOString()}).eq('id',editing.id):await sb.from('quotations').insert({...p,is_active:true})
    ...
    <div className="p-4 md:p-8 min-h-screen">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          <ImportExportBar table="quotations" filename="quotations" columns={[
            { header: 'Quote Number', dbKey: 'quote_number', example: 'Q-2024-001', required: true },
            ...
            { header: 'Quote Date', dbKey: 'quote_date', example: '2024-01-15' },
            { header: 'Expiry Date', dbKey: 'expiry_date', example: '2024-02-15' },
            { header: 'Status', dbKey: 'status', example: 'Draft' },
            { header: 'Total', dbKey: 'total', example: '1250.00' },
            { header: 'Notes', dbKey: 'notes', example: '' },
          ]} onImportDone={load} />
          <button onClick={openAdd} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-medium px-4 py-2.5 rounded-lg transition-colors"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4"/></svg>Add Quotation</button>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900 overflow-x-auto">
        ...
        :<table className="w-full min-w-[600px] text-sm"><thead><tr className="border-b border-gray-800">{['Quote #','Customer','Quote Date','Expiry Date','Status','Total'].map(h=><th key={h} className="text-left text-xs font-semibold text-gray-500 px-5 py-3">{h}</th>)}</tr></thead>
        <tbody>{filtered.map((r,i)=><tr key={r.id} onClick={()=>openEdit(r)} className={`border-b border-gray-800/60 last:border-0 cursor-pointer hover:bg-gray-800/40 transition-colors ${i%2===0?'':'bg-gray-800/10'}`}>
          ...
          <td className="px-5 py-3.5 text-gray-300 font-medium">{fmt$(r.total)}</td>
        </tr>)}</tbody></table>}
      </div>
      ...
      <div ref={ref} onClick={(e)=>e.stopPropagation()} className={`fixed inset-0 md:inset-auto md:top-0 md:right-0 md:h-full w-full md:max-w-md bg-gray-900 border-l border-gray-800 z-50 flex flex-col shadow-2xl transition-transform duration-300 ease-in-out ${open?'translate-x-0':'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div><label className="block text-xs text-gray-400 mb-1.5">Total ($)</label><input type="number" value={form.total} onChange={e=>setForm(p=>({...p,total:e.target.value}))} className={inp}/></div>
