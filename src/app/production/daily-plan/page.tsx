...
const SC: Record<string,string> = { Planned:'bg-gray-700/40 text-gray-400 border-gray-700', 'In Progress':'bg-blue-500/15 text-blue-400 border-blue-500/20', Complete:'bg-emerald-500/15 text-emerald-400 border-emerald-500/20', Delayed:'bg-red-500/15 text-red-400 border-red-500/20' }
    ...
    <div className="p-4 md:p-8 min-h-screen">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-white mt-1">Daily Plan</h1>
      ...
      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-sm"><svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg><input placeholder="Search plans…" value={search} onChange={e=>setSearch(e.target.value)} className="w-full bg-gray-900 border border-gray-800 text-white placeholder-gray-600 rounded-lg pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"/></div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900 overflow-x-auto">
        ...
        :<table className="w-full min-w-[600px] text-sm"><thead><tr className="border-b border-gray-800">{['Date','Work Order','Machine','Target Qty','Actual Qty','Variance','Status'].map(h=><th key={h} className="text-left text-xs font-semibold text-gray-500 px-5 py-3">{h}</th>)}</tr></thead>
        <tbody>{filtered.map((r,i)=>{
          ...
          return <tr key={r.id} onClick={()=>openEdit(r)} className={`border-b border-gray-800/60 last:border-0 cursor-pointer hover:bg-gray-800/40 transition-colors ${i%2===0?'':'bg-gray-800/10'}`}>
            <td className="px-5 py-3.5 text-white font-medium">{fmtD(r.plan_date)}</td>
