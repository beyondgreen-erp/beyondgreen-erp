...
interface PlanRow {
  ...
  uom: string; packaging: string; done: number; prodStatus: string
}
...
export default function ShippingQueuePage() {
  ...
  const load = useCallback(async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const mapped: QueueItem[] = rows.map((o: any) => ({ id: o.id, sales_order_id: o.id, status: o.status, sales_orders: o }))
    setItems(mapped)
    setLoading(false)
  }, [])
  ...
  async function openOrder(item: QueueItem) {
    ...
    // Robust: fetch lines (no embed), then products separately.
    ...
    const rowsOut: PlanRow[] = ls.map((l: any) => {
      ...
      return {
        ...
        prodStatus: l.production_status || '',
      }
    })
  ...
  function genCaseLabels() {
    ...
    buildCaseLabels({ poNumber: activeItem?.sales_orders?.po_number || '', shipToName: st.name, shipToAddress: st.addr }, cases).save(`case-labels-${activeItem?.sales_orders?.order_number || 'order'}.pdf`)
  }

  function genPalletLabels() {
    const st = shipTo(activeItem?.sales_orders)
    const pallets: PalletLabel[] = []
    const perPallet = Math.max(1, cap); const total = totals.pallets; let caseCursor = totals.cases
    for (let p = 1; p <= total; p++) { const cnt = Math.min(perPallet, caseCursor); caseCursor -= cnt; pallets.push({ palletNumber: p, totalPallets: total, caseCount: cnt, weight: +(totals.weight / total).toFixed(0) }) }
    buildPalletLabels({ poNumber: activeItem?.sales_orders?.po_number || '', shipToName: st.name, shipToAddress: st.addr }, pallets).save(`pallet-labels-${activeItem?.sales_orders?.order_number || 'order'}.pdf`)
  }
  ...
  async function genBOL() {
    ...
    const data: BolData = {
      ...
      shipToName: st.name, shipToAddress: st.addr, carrierName: o?.carrier || 'SINGLE (Walmart FLEET)', scac: 'WALM',
    ...
    buildBOL(data, [line], logo).save(`${bolNumber}.pdf`)
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <p className="text-xs text-gray-400 mb-5">Mirrors the Sales Orders board. Showing orders with status: {SHIPPABLE.join(', ')}.</p>
              ...
              <div key={item.id} className="rounded-xl border border-gray-200 bg-white">
                ...
                {open && (
                  <div className="border-t border-gray-100 p-4">
                    {/* Order info — mirrors the sales order record */}
                      ...
                      <>
                        ...
                        <table className="w-full text-xs mb-3">
                          <thead><tr className="text-left text-gray-500 border-b bg-gray-50">
                            ...
                            <th className="text-right px-2">Done</th><th className="px-2">UOM</th><th className="px-2">Packaging</th>
                            <th className="text-right px-2">Units/Case</th><th className="text-right px-2">Cases</th><th className="text-right px-2">Case lb</th><th className="text-center px-2">UPC</th>
                          </tr></thead>
                          <tbody>
                            {plan.map((r, i) => (
                              <tr key={i} className="border-b border-gray-50">
                                ...
                                <td className="text-right px-2 text-gray-400">{r.done || '—'}</td>
                                <td className="px-2 text-gray-500">{r.uom || '—'}</td>
                                <td className="px-2 text-gray-500">{r.packaging || '—'}</td>
                                <td className="text-right px-2">{r.unitsPerCase}</td>
                                <td className="text-right px-2"><input type="number" value={r.cases} onChange={e => updateCases(i, Number(e.target.value))} className="w-16 border rounded px-1 py-0.5 text-right" /></td>
                                <td className="text-right px-2">{r.caseWeightLb}</td>
                    ...
                    {/* Shared Activity Log — same record as the Sales Orders board */}
