  ...
  return (
    <div className="p-5 md:p-8 min-h-screen">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 bg-violet-600 hover:bg-violet-500 text-white text-sm font-medium px-4 py-2.5 rounded-xl transition-colors">
      ...
      {/* Slide-out panel */}
      <div ref={panelRef} className={`fixed inset-0 md:inset-auto md:top-0 md:right-0 md:h-full w-full md:w-[560px] bg-[#111113] border-l border-[#2A2A35] z-50 flex flex-col shadow-2xl transition-transform duration-300 ease-in-out ${(selected || adding) ? 'translate-x-0' : 'translate-x-full'}`} onClick={e => e.stopPropagation()}>
        {/* Add / Edit Form */}
        {(adding || (selected && editMode)) && (
          <>
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#2A2A35] shrink-0">
              <h2 className="text-white font-semibold">{adding ? 'Add Lot Code' : 'Edit Lot Code'}</h2>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
              ...
              {[
                ...
                <div key={f.key}>
                  ...
                  <input type={f.type} value={(form as any)[f.key]} onChange={e => setForm(p => ({...p, [f.key]: e.target.value}))}
                    className="w-full bg-[#18181C] border border-[#2A2A35] rounded-xl px-3 py-2.5 text-sm text-white placeholder-[#5A5A6A] focus:outline-none focus:border-[#00C896] transition-all"/>
                </div>
              ))}
              <div>
                ...
                <select value={form.lot_type} onChange={e => setForm(p => ({...p, lot_type: e.target.value}))}
                  className="w-full bg-[#18181C] border border-[#2A2A35] rounded-xl px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#00C896] transition-all cursor-pointer">
              ...
              <div>
                ...
                <textarea rows={3} value={form.notes} onChange={e => setForm(p => ({...p, notes: e.target.value}))}
                  className="w-full bg-[#18181C] border border-[#2A2A35] rounded-xl px-3 py-2.5 text-sm text-white placeholder-[#5A5A6A] focus:outline-none focus:border-[#00C896] transition-all resize-none"/>
              </div>
            </div>
            <div className="px-6 py-4 border-t border-[#2A2A35] shrink-0 flex gap-3">
              ...
              <button onClick={close} className="px-5 text-sm rounded-xl border border-[#2A2A35] text-[#5A5A6A] hover:text-white transition-colors">Cancel</button>
              <button onClick={handleSave} disabled={saving} className="flex-1 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-white text-sm font-medium py-2.5 rounded-xl flex items-center justify-center gap-2">
          ...
          <>
            <div className="flex items-center justify-between px-6 py-5 border-b border-[#2A2A35] shrink-0">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => openEdit(selected)} className="text-xs px-3 py-1.5 rounded-lg border border-[#2A2A35] text-[#9898A8] hover:text-white hover:border-gray-600 transition-colors">Edit</button>
                <button onClick={close} className="p-2 rounded-xl text-[#5A5A6A] hover:text-white hover:bg-[#18181C] transition-colors">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
            ...
            <div className="px-6 py-4 border-t border-[#2A2A35] shrink-0 flex gap-3">
              ...
              <button onClick={close} className="flex-1 text-sm px-4 py-2.5 rounded-xl border border-[#2A2A35] text-[#5A5A6A] hover:text-white transition-colors">Close</button>
              <button onClick={() => openEdit(selected)} className="px-5 bg-violet-600 hover:bg-violet-500 text-white text-sm font-medium py-2.5 rounded-xl transition-colors">Edit</button>
            </div>
          </>
        )}
      </div>
