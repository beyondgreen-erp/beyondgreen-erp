...
function StatCard({ label, value, sub, color, href }: { label: string; value: string | number; sub?: string; color: string; href?: string }) {
  const content = (
    <div className={`bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-gray-700 transition-colors ${href?'cursor-pointer':''}`}>
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-semibold text-white mt-1">Production Overview</h1>
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <StatCard label="Active Work Orders" value={stats.activeWOs} color="text-white" href="/production" sub="Not yet complete"/>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <StatCard label="Total Machines" value={stats.totalMachines} color="text-white" href="/production/machine-status"/>
              <StatCard label="Uptime Rate" value={stats.totalMachines>0?`${Math.round(((stats.totalMachines-stats.machinesDown)/stats.totalMachines)*100)}%`:'—'} color="text-white" sub="Machines not down"/>
            </div>
          </div>
          <div>
