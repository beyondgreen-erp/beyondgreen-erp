...
export default function CapacityPlanPage() {
  ...
  async function handleDelete(){if(!editing)return;if(!confirm('Permanently delete this capacity entry? This cannot be undone.'))return;const{error}=await sb.from('capacity_plan').delete().eq('id',editing.id);if(error){alert('Delete failed: '+error.message);return}close();load()}
    ...
    <div className="p-4 md:p-8 min-h-screen">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-[#1A1D2E] mt-1">Capacity Plan</h1>
      ...
      <div className="rounded-xl border border-[#E4E6EE] bg-white overflow-x-auto">
        ...
        :<table className="w-full min-w-[600px] text-sm"><thead><tr className="border-b border-[#E4E6EE]">{['Machine','Week Starting','Available Hrs','Booked Hrs','Utilization','Notes'].map(h=><th key={h} className="text-left text-xs font-semibold text-gray-500 px-5 py-3">{h}</th>)}</tr></thead>
        <tbody>{filtered.map((r,i)=>{
          const pct=r.available_hours>0?Math.round((r.booked_hours/r.available_hours)*100):0
          return <tr key={r.id} onClick={()=>openEdit(r)} className={`border-b border-[#E4E6EE]/60 last:border-0 cursor-pointer transition-colors ${utilBg(pct)||( i%2===0?'hover:bg-[#F9FAFB]':'bg-[#F5F6FA]/10 hover:bg-[#F9FAFB]')}`}>
            <td className="px-5 py-3.5 text-[#1A1D2E] font-medium">{r.machine_id?mmap[r.machine_id]||'—':'—'}</td>
            ...
            <td className="px-5 py-3.5">
              <div className="flex items-center gap-2">
                <div className="w-20 h-1.5 bg-[#F9FAFB] rounded-full overflow-hidden"><div className={`h-full rounded-full transition-all ${pct>=90?'bg-red-500':pct>=70?'bg-amber-500':'bg-emerald-500'}`} style={{width:`${Math.min(pct,100)}%`}}/></div>
      ...
      <div ref={ref} onClick={(e)=>e.stopPropagation()} className={`fixed inset-0 md:inset-auto md:top-0 md:right-0 md:h-full w-full md:max-w-md bg-white border-l border-[#E4E6EE] z-50 flex flex-col shadow-2xl transition-transform duration-300 ease-in-out ${open?'translate-x-0':'translate-x-full'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          {(parseFloat(form.available_hours)||0)>0&&(
            <div className={`rounded-lg px-3 py-2.5 flex items-center justify-between ${formUtil>=90?'bg-red-500/10 border border-red-500/20':formUtil>=70?'bg-amber-500/10 border border-amber-500/20':'bg-[#F5F6FA]/50'}`}>
