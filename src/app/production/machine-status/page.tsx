...
const SC: Record<string,string> = { Running:'bg-emerald-500/15 text-emerald-400 border-emerald-500/20', Idle:'bg-gray-700/40 text-gray-400 border-gray-700', Maintenance:'bg-amber-500/15 text-amber-400 border-amber-500/20', Down:'bg-red-500/15 text-red-400 border-red-500/20' }
...
export default function MachineStatusPage() {
  ...
  async function toggleArchive(){if(!editing)return;setBusy(true);await sb.from('machines').update({is_active:!editing.is_active,updated_at:new Date().toISOString()}).eq('id',editing.id);setBusy(false);close();load()}
  async function handleDelete(){if(!editing)return;if(!confirm('Permanently delete this machine? This cannot be undone.'))return;const{error}=await sb.from('machines').delete().eq('id',editing.id);if(error){alert('Delete failed: '+error.message);return}close();load()}
  ...
  const counts=STATUSES.map(s=>({s,n:rows.filter(r=>r.is_active&&r.status===s).length}))
    ...
    <div className="p-4 md:p-8 min-h-screen">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-white mt-1">Machine Status</h1>
          <p className="text-gray-500 text-sm mt-0.5">{loading?'Loading…':`${filtered.length} machine${filtered.length!==1?'s':''}`}</p>
        </div>
        <div className="flex items-center gap-2">
          <ImportExportBar table="machines" filename="machines" columns={[
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {counts.map(({s,n})=><div key={s} className="bg-gray-900 border border-gray-800 rounded-xl p-4"><div className="flex items-center gap-2 mb-1"><div className={`w-2 h-2 rounded-full ${DOT[s]}`}/><span className="text-xs text-gray-500">{s}</span></div><p className={`text-2xl font-bold ${s==='Running'?'text-emerald-400':s==='Down'?'text-red-400':s==='Maintenance'?'text-amber-400':'text-gray-400'}`}>{n}</p></div>)}
      </div>
      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-sm"><svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg><input placeholder="Search machines…" value={search} onChange={e=>setSearch(e.target.value)} className="w-full bg-gray-900 border border-gray-800 text-white placeholder-gray-600 rounded-lg pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"/></div>
      ...
      <div className="rounded-xl border border-gray-800 bg-gray-900 overflow-x-auto">
        ...
        :<table className="w-full min-w-[600px] text-sm"><thead><tr className="border-b border-gray-800">{['Machine Name','Code','Status','Location','Notes'].map(h=><th key={h} className="text-left text-xs font-semibold text-gray-500 px-5 py-3">{h}</th>)}</tr></thead>
