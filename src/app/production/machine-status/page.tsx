    ...
    <div className="p-4 md:p-8 min-h-screen">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-[#1A1D2E] mt-1">Machine Status</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {counts.map(({s,n})=><div key={s} className="bg-white border border-[#E4E6EE] rounded-xl p-4"><div className="flex items-center gap-2 mb-1"><div className={`w-2 h-2 rounded-full ${DOT[s]}`}/><span className="text-xs text-gray-500">{s}</span></div><p className={`text-2xl font-bold ${s==='Running'?'text-emerald-400':s==='Down'?'text-red-400':s==='Maintenance'?'text-amber-400':'text-gray-400'}`}>{n}</p></div>)}
      </div>
      <div className="flex items-center gap-3 mb-4">
        <div className="relative flex-1 max-w-sm"><svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg><input placeholder="Search machines…" value={search} onChange={e=>setSearch(e.target.value)} className="w-full bg-white border border-[#E4E6EE] text-[#1A1D2E] placeholder-[#9CA3AF] rounded-lg pl-9 pr-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition"/></div>
