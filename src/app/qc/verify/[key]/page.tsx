...
const RESULT_CFG: Record<string, { label: string; cls: string; icon: string }> = {
  ...
  Pending: { label: 'PENDING', cls: 'text-[#9898A8] border-[#3A3A45] bg-[#2A2A35]', icon: '○' },
}
...
export default function QCVerifyPage() {
    ...
    return (
      <div className="min-h-screen bg-[#0A0A0B] flex items-center justify-center p-8">
        <div className="text-center max-w-sm">
          ...
          <h1 className="text-white font-bold text-xl mb-2">QC Record Not Found</h1>
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0B] flex items-center justify-center p-6">
      <div className="w-full max-w-md bg-[#111113] border border-[#2A2A35] rounded-2xl overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="px-6 py-5 border-b border-[#2A2A35]">
          <div className="flex items-center gap-2.5 mb-1">
            ...
            <span className="text-white font-bold text-sm">beyondGREEN ERP</span>
          </div>
        ...
        <div className="px-6 pb-6 space-y-3">
          {[
            ...
            <div key={f.label} className="flex items-center justify-between py-2 border-b border-[#2A2A35] last:border-0">
              <span className="text-[#9898A8] text-xs">{f.label}</span>
              <span className="text-white text-xs font-medium">{f.value}</span>
            </div>
          ))}
          {inspection.notes && (
            <div className="bg-[#18181C] border border-[#2A2A35] rounded-xl p-3 mt-2">
              <p className="text-[#5A5A6A] text-[11px] uppercase tracking-wider mb-1">Notes</p>
              <p className="text-white text-xs">{inspection.notes}</p>
            </div>
          )}
        </div>

        <div className="px-6 pb-4 text-center">
          <p className="text-[#3A3A45] text-[10px]">Verified by beyondGREEN ERP · ID: {inspection.id.slice(0, 8).toUpperCase()}</p>
        </div>
      </div>
    </div>
  )
