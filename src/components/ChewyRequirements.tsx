...
export default function ChewyRequirements() {
  ...
  useEffect(() => {
    ...
    const ch = sb.channel('chewy-po-requirements-sync')
  ...
  const perPo = useMemo(() => {
    ...
    for (const o of orders) {
      const poLabel = (o.po_number || o.order_number || '—')
      const shipKey = o.required_ship_date ? Date.parse(o.required_ship_date) : Number.POSITIVE_INFINITY
      ...
      for (const [sku, need] of Object.entries(reqMap)) {
        ...
        out.push({ po: poLabel, poName: o.order_number || '', shipKey, sku, name: cp?.product_name ?? null, need: Math.round(need * 100) / 100, onHand: Number(cp?.on_hand_qty ?? NaN), short: null })
      }
    }
  ...
  function exportPdf() {
    ...
    doc.text('Chewy PO Requirements', M, y + 18)
    ...
    doc.text(groups.length + ' open POs  ·  ' + shortLineItems + ' short line items across ' + posWithShort + ' POs  ·  shipped POs excluded', M, y + 46)
    y += 72
    ...
    const barChart = (title: string, items: [string, number][], suffix: string) => {
      ...
      if (items.length === 0) { doc.setFont('helvetica', 'normal'); doc.setFontSize(9); doc.setTextColor(120, 120, 120); doc.text('None — all items in stock.', M, y + 4); y += 24; return }
    ...
    barChart('Top Item Shortages (units short)', topComp, '')
    barChart('Short Items by PO', poShort, ' short')
    ...
    autoTable(doc, {
      startY: y,
      head: [['PO', 'Item', 'Description', 'Qty Needed', 'On Hand', 'Short']],
      body: bodyRows,
      theme: 'grid',
    ...
    doc.save('beyondGREEN_Chewy_PO_Requirements_' + new Date().toISOString().slice(0, 10) + '.pdf')
  }
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#E4E6EE] overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-6 py-4 border-b border-[#E4E6EE]">
        <div>
          <h2 className="text-lg font-bold text-[#1A1D2E]">Chewy PO Requirements</h2>
          <p className="text-xs text-gray-500 mt-0.5">{loading ? 'Loading…' : `${groups.length} open PO${groups.length === 1 ? '' : 's'}`} · click a PO to see its item requirements · shipped POs drop off</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => load()} disabled={loading} title="Reload latest Chewy PO data" className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border border-[#E4E6EE] text-gray-600 hover:bg-[#F5F6FA] disabled:opacity-50 transition-colors"><i className={'ti ti-refresh' + (loading ? ' animate-spin' : '')} />Refresh</button>
          <button onClick={exportPdf} disabled={loading || groups.length === 0} title="Download a PDF report with charts" className="flex items-center gap-1.5 px-3 py-2 text-sm font-semibold rounded-lg bg-[#1C49C2] text-white hover:bg-[#163a9c] disabled:opacity-50 transition-colors"><i className="ti ti-file-type-pdf" />Export PDF</button>
          <div className="relative">
            <i className="ti ti-search absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm" />
            <input placeholder="Search PO or item…" value={q} onChange={e => setQ(e.target.value)} className="pl-9 pr-4 py-2 text-sm bg-white border border-[#E4E6EE] rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
      </div>
      <div>
        {loading ? <div className="px-6 py-16 text-center text-gray-400 text-sm">Loading…</div>
         : groups.length === 0 ? <div className="px-6 py-16 text-center text-gray-400 text-sm">No item requirements for open Chewy POs.</div>
         : groups.map(g => {
          ...
          return (
            <div key={g.po} className="border-b border-[#EEF0F4] last:border-b-0">
              <button onClick={() => setOpen(o => ({ ...o, [g.po]: !o[g.po] }))} className="w-full flex items-center gap-3 px-4 sm:px-6 py-3 hover:bg-[#F8FAFC] text-left transition-colors">
                ...
                <span className="ml-auto text-[11px] font-medium text-gray-500 shrink-0">{g.rows.length} item{g.rows.length === 1 ? '' : 's'}</span>
                {g.shortCount > 0
