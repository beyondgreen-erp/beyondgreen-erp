...
import { uploadFile, downloadFile, getFileUrl, formatFileSize, fileIcon } from '@/lib/fileHelpers'
...
export default function FileUpload({
  ...
  async function handleView(att: FileAttachment) {
    const url = await getFileUrl(supabase, att.storage_path)
            ...
            <div key={att.id} className="flex items-center gap-2.5 bg-[#F5F6FA]/60 rounded-lg px-3 py-2.5 group">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  ...
                  title="View"
                  className="p-1.5 text-gray-500 hover:text-[#3B6FE0] transition-colors rounded"
                >
