      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-0.5">
          {NAV.map((section, si) => (
            <div key={section.label} className={si > 0 ? 'mt-4' : ''}>
              {!collapsed && (
                <p
                  className="text-xs font-semibold px-2 mb-1.5 select-none"
                  style={{ color: '#4D6080', letterSpacing: '0.05em' }}
                >
                  {section.label}
                </p>
              )}
              ...
              {section.items.map(item => {
                ...
                return (
                  <Link
                    ...
                    style={{
                      background: active ? 'rgba(59,111,224,0.18)' : 'transparent',
                      color: active ? '#FFFFFF' : '#8A9FC0',
                    }}
                    onMouseEnter={e => {
                      if (!active) {
                        (e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.06)'
                        ;(e.currentTarget as HTMLElement).style.color = '#C8D5E8'
                      }
                    }}
                    onMouseLeave={e => {
                      if (!active) {
                        (e.currentTarget as HTMLElement).style.background = 'transparent'
                        ;(e.currentTarget as HTMLElement).style.color = '#8A9FC0'
                      }
                    }}
                  >
                    {active && (
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 rounded-r-full" style={{ background: '#3B6FE0' }}/>
                    )}
        ...
        <div className="shrink-0 p-3" style={{ borderTop: '1px solid rgba(255,255,255,0.07)' }}>
          ...
          <button
            ...
            onMouseLeave={e => {
              ;(e.currentTarget as HTMLElement).style.color = '#5A6E8A'
              ;(e.currentTarget as HTMLElement).style.background = 'transparent'
            }}
          >
