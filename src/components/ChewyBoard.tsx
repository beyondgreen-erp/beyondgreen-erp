...
function statusForGroup(group: string): string | null {
  switch (group) {
    ...
    case 'Chewy Orders': return 'Confirmed'
    default: return null
  }
}
...
export default function ChewyBoard() {
  ...
  const CHEWY_SKU_RE = /^/  // Chewy SKUs have no fixed prefix; accept any
  const chewyProducts = useMemo(() => products.filter(p => CHEWY_SKU_RE.test(p.sku || '')), [products])
  const isChewySku = (sku?: string | null) => CHEWY_SKU_RE.test(String(sku || '').trim())

  const load = useCallback(async () => {
    setLoading(true)
    const { data: o } = await sb.from('chewy_board_orders').select('*').eq('archived', false).order('board_position', { ascending: true, nullsFirst: false }).order('name', { ascending: true })
    const orders = (o as WOrder[]) || []
    setRows(orders)
    const { data: l } = await sb.from('chewy_board_lines').select('*').order('line_number', { ascending: true })
    ...
    setBom((bm as BomRow[]) || [])
    const { data: pl } = await sb.from('chewy_pallets').select('*').order('pallet_number', { ascending: true })
    const pm: Record<string, Pallet[]> = {}
    ;((pl as Pallet[]) || []).forEach(p => { (pm[p.order_id] ||= []).push(p) })
    setPallets(pm)
    const { data: pi } = await sb.from('chewy_pallet_items').select('*')
    ...
    if (ids.length) {
      ...
      for (let i = 0; i < ids.length; i += 200) {
        ...
        const { data: cm } = await sb.from('comments').select('record_id').eq('record_type', 'chewy_order').in('record_id', slice)
        ;((cm as any[]) || []).forEach(c => { cc[c.record_id] = (cc[c.record_id] || 0) + 1 })
        const { data: fm } = await sb.from('file_attachments').select('record_id').eq('record_type', 'chewy_order').in('record_id', slice)
  ...
  async function saveRecord() {
    ...
    try {
      ...
      const { error } = await sb.from('chewy_board_orders').update(patch).eq('id', detail.id)
      ...
      if (toDelete.length) await sb.from('chewy_board_lines').delete().in('id', toDelete)
      for (let i = 0; i < lineForms.length; i++) {
        ...
        if (l._new || !l.id) await sb.from('chewy_board_lines').insert(row)
        else await sb.from('chewy_board_lines').update(row).eq('id', l.id)
      }
      if (groupForStatus(patch.status) === 'Shipped' && !detail.shipment_id) {
        const sid = await shipChewyOrder({ ...detail, ...patch } as WOrder)
  ...
  async function deleteRecord() {
    ...
    setDeleting(true)
    try {
      await sb.from('chewy_pallets').delete().eq('order_id', detail.id)
      await sb.from('chewy_board_lines').delete().eq('order_id', detail.id)
      try { await sb.rpc('delete_record_comments', { p_record_type: 'chewy_order', p_record_id: detail.id }) } catch { /* */ }
      const { error } = await sb.from('chewy_board_orders').delete().eq('id', detail.id)
  ...
  async function uploadPO(file: File) {
    ...
    try {
      const path = `chewy-po/${Date.now()}_${file.name.replace(/[^A-Za-z0-9._-]/g, '_')}`
      ...
      const o = j.order || {}
      const shipTo = o.ship_to || (o.dc_number ? ('Chewy DC ' + o.dc_number) : '')
      setNf((f: any) => ({
        ...f,
        po_number: o.po_number || f.po_number,
        name: nfTouched.name ? f.name : (o.po_number ? `CHEWY|${o.po_number}` : f.name),
  ...
  function onPoChange(v: string) {
    setNf((f: any) => ({ ...f, po_number: v, name: nfTouched.name ? f.name : (v.trim() ? `CHEWY|${v.trim()}` : '') }))
  }
  async function createOrder() {
    const name = (nf.name || '').trim() || (nf.po_number ? `CHEWY|${String(nf.po_number).trim()}` : '')
    ...
    try {
      ...
      const ins: any = {
        name, group_name: 'Chewy Orders', status: 'Pending',
      ...
      const { data, error } = await sb.from('chewy_board_orders').insert(ins).select('*').single()
      ...
      if (toInsert.length) await sb.from('chewy_board_lines').insert(toInsert)
      setShowNew(false)
      await load()
  ...
  async function generatePallets(order: WOrder) {
    ...
    try {
      ...
      const { error } = await sb.from('chewy_pallets').insert(toIns)
      if (error) { alert('Could not generate pallets: ' + error.message); return }
      if (base) await sb.from('chewy_pallets').update({ total_pallets: totalAfter }).eq('order_id', order.id)
      await load()
  ...
  async function deletePallet(p: Pallet) {
    ...
    await sb.from('chewy_pallets').delete().eq('id', p.id)
    await load()
  }
  async function shipChewyOrder(order: WOrder): Promise<string | null> {
    ...
    await sb.from('chewy_board_orders').update({ shipment_id: sid }).eq('id', order.id)
    return sid
  }
  ...
  async function markShipped(order: WOrder) {
    ...
    await sb.from('chewy_board_orders').update({ status: 'Shipped', group_name: 'Shipped', updated_at: new Date().toISOString() }).eq('id', order.id)
    const sid = await shipChewyOrder(order)
...
<body><div class="toolbar noprint"><button onclick="window.print()" style="padding:8px 16px;font-size:14px">Print labels (4\u00d76)</button></div>${labels.join('')}</body></html>`
  ...
  async function moveToGroup(id: string, targetGroup: string) {
    ...
    if ((r.group_name || 'Chewy Orders') === targetGroup) return
    ...
    const { error } = await sb.from('chewy_board_orders').update(patch).eq('id', id)
    if (error) { alert('Move failed: ' + error.message); await load() }
    else if (targetGroup === 'Shipped' && !r.shipment_id) { await shipChewyOrder({ ...r, ...patch } as WOrder); await load() }
  }

  // ── Requirements roll-up ───────────────────────────────────────────────────
  const req = useMemo(() => {
    const included = rows.filter(r => (r.group_name || 'Chewy Orders') !== 'Cancelled')
    ...
    // components requirement (packaging / BOM) from chewy_bom
  ...
  const groupRows = (key: string) => rows
    .filter(r => (r.group_name || 'Chewy Orders') === key && match(r))
    .sort((a, b) => shipSortKey(b) - shipSortKey(a))
  const extra = Array.from(new Set(rows.map(r => r.group_name || 'Chewy Orders').filter(k => k && !GROUPS.some(g => g.key === k))))
  ...
  // Chewy sell price per SRP: prefer an explicit line price, else the product's
  // unit → case → distribution → wholesale price (distribution_price is the Chewy contract price).
  ...
  function lineWarn(l: WLine) {
    ...
    if (!sku) return null
    if (!isChewySku(sku)) return <span className="text-[11px] text-red-500">not a Chewy SKU</span>
    const p = skuInfo(sku)
  ...
  return (
    <div ref={navRef}>
      <ExportButton rows={rows} name="Chewy Orders" />
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4">
        <div>
          <span className="mon-tag">🛒 Chewy Orders</span>
          <p className="text-gray-500 text-sm mt-1">{loading ? 'Loading…' : `${shown} of ${rows.length} orders · ${fmt$(totalVal)} total value`}</p>
        </div>
        <button onClick={openNew} className="mon-btn">+ New Chewy order</button>
      </div>
          ...
          <div className="relative w-full max-w-[720px] my-6 bg-white rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 py-4 max-h-[76vh] overflow-y-auto space-y-4">
              <div className="rounded-lg border-2 border-dashed border-[#CBD5E1] bg-[#F8FAFC] p-3">
                <div className="flex items-center gap-3 flex-wrap">
                  <label className={`text-sm font-semibold px-3 py-1.5 rounded-lg ${poParsing ? 'bg-gray-100 text-gray-400 cursor-wait' : 'bg-[#EAF0FC] text-[#0086C0] hover:bg-[#DCE7FB] cursor-pointer'}`}>
                    {poParsing ? 'Reading PO…' : '📄 Upload Chewy PO'}
              ...
              <div>
                ...
                <datalist id="wm-skus">{chewyProducts.map(p => <option key={p.id} value={p.sku}>{p.product_name || ''}</option>)}</datalist>
              </div>
            </div>
          ...
          <div className="relative w-full max-w-[900px] my-6 bg-white rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between px-6 py-4 text-white" style={{ background: statusColor(detail.status).solid || '#0086C0' }}>
              <div className="min-w-0">
                <p className="text-white/70 text-xs uppercase tracking-wide">Chewy Order · {detail.group_name || '—'}</p>
            ...
            <div className="px-6 py-4 max-h-[76vh] overflow-y-auto space-y-5">
              ...
              <div className="border-t border-[#EEF0F4] pt-4">
                <FileUpload supabase={sb} recordType="chewy_order" recordId={detail.id} currentUserEmail={userEmail} />
              </div>
              <div className="border-t border-[#EEF0F4] pt-4">
                <Comments recordId={detail.id} recordType="chewy_order" currentUserEmail={userEmail} title="Notes & Comments" />
              </div>
            </div>
          </div>
        </div>
