  ...
  function genPackingList() {
    ...
    pallets.forEach(p => {
      ...
      plan.filter(r => (r.alloc[p.id] || 0) > 0).forEach(r => {
        ...
        const cur = bySku.get(r.sku) || { sku: r.sku, description: r.description, caseCount: 0, unitsInCase: r.unitsPerCase, units: 0, weight: 0 }
        cur.caseCount += cs
        cur.units = (cur.units || 0) + cs * r.unitsPerCase
