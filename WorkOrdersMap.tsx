...
const STATE_CENTER: Record<string, [number, number]> = {
  ...
  ME:[44.693947,-69.381927], MD:[39.063946,-76.6122], MA:[42.230171,-71.530106],
...
function getCoords(wo: WorkOrderPoint): [number, number] | null {
  if (wo.city && wo.state) {
    const key = `${wo.city}_${wo.state}`
    if (CITY_COORDS[key]) return CITY_COORDS[key]
    // Try state center as fallback
    if (STATE_CENTER[wo.state]) return STATE_CENTER[wo.state]
  } else if (wo.state && STATE_CENTER[wo.state]) {
    return STATE_CENTER[wo.state]
  }
  return null
}
...
function MapCounter({ mapped, total }: { mapped: number; total: number }) {
  ...
  useEffect(() => {
    ...
    el.textContent = mapped === total
      ? `${total} work order${total !== 1 ? 's' : ''} mapped`
      : `${mapped} of ${total} work order${total !== 1 ? 's' : ''} mapped`
...
export default function WorkOrdersMap({ workOrders }: Props) {
  const withCoords = workOrders
    .map(wo => ({ wo, coords: getCoords(wo) }))
    .filter((x): x is { wo: WorkOrderPoint; coords: [number, number] } => x.coords !== null)

  // Aggregate work orders by location
  const locationAgg: Record<string, { lat: number; lng: number; orders: WorkOrderPoint[] }> = {}
  withCoords.forEach(({ wo, coords }) => {
    const key = wo.city && wo.state ? `${wo.city}_${wo.state}` : `${coords[0]}_${coords[1]}`
    if (!locationAgg[key]) {
      locationAgg[key] = { lat: coords[0], lng: coords[1], orders: [] }
    }
    locationAgg[key].orders.push(wo)
  })
    ...
    <div style={{ height: '100%', width: '100%', minHeight: 480, display: 'flex', flexDirection: 'column' }}>
      <div style={{ flex: 1, minHeight: 480 }}>
        <MapContainer center={[39.5, -98.35]} zoom={4} style={{ height: '100%', width: '100%', minHeight: 480, borderRadius: 8 }}>
          ...
          />
          <MapCounter mapped={withCoords.length} total={workOrders.length} />

          {withCoords.map(({ wo, coords }, i) => (
            <CircleMarker key={wo.id || i} center={coords} radius={8}
              pathOptions={{ fillColor: getStatusColor(wo.status), fillOpacity: 0.85, color: '#fff', weight: 2 }}>
              <Popup>
                <div style={{ fontSize: 12, minWidth: 200 }}>
                  <div style={{ fontWeight: 700, marginBottom: 4 }}>{wo.customer_name || 'Unknown Customer'}</div>
                  <div style={{ marginBottom: 8, paddingBottom: 8, borderBottom: '1px solid #e5e7eb' }}>
                    <div style={{ fontWeight: 600, marginBottom: 2 }}>Work Order: {wo.wo_number}</div>
                    <div>Status: <span style={{ fontWeight: 600, color: getStatusColor(wo.status) }}>{wo.status}</span></div>
                  </div>
                  <div>Location: {[wo.city, wo.state].filter(Boolean).join(', ') || 'Unknown'}</div>
                </div>
              </Popup>
            </CircleMarker>
          ))}
      ...
      {/* Legend */}
      <div style={{ display: 'flex', gap: 12, marginTop: 12, flexWrap: 'wrap', fontSize: 11, alignItems: 'center', padding: '12px 8px' }}>
        {Object.entries(STATUS_COLORS).map(([status, color]) => (
          <div key={status} style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#6b7280' }}>
            <div style={{ width: 12, height: 12, borderRadius: '50%', background: color, flexShrink: 0, border: '1px solid white' }} />
            {status}
          </div>
        ))}
        {withCoords.length < workOrders.length && (
          <span style={{ color: '#6b7280', marginLeft: 'auto' }}>
            {workOrders.length - withCoords.length} work order{workOrders.length - withCoords.length !== 1 ? 's' : ''} without location data not shown
          </span>
        )}
      </div>
    </div>
