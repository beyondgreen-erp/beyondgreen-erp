...
export default function ProofPortal() {
  ...
  const [tab, setTab] = useState<'comments' | 'downloads'>('comments')
  ...
  const fetchData = useCallback(async (first = false) => {
    const r = await fetch(`/api/proof/${token}${first ? '?view=1' : ''}`, { cache: 'no-store' })
  ...
  useEffect(() => {
    ...
    const t = setInterval(async () => {
      ...
      setData(prev => prev ? { ...prev, comments: p.comments, files: p.files, link: p.link, design: p.design } : p)
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="space-y-1.5">
                    {data.files.map(f => (
                      <a key={f.id} href={f.url || '#'} className="flex items-center gap-2 p-2 rounded-lg border border-gray-200 hover:border-gray-400 text-sm">
